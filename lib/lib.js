...
Const = {
    ...
    GCM_ID : "AIzaSyAeF4dV4kgZ8vMQVdjv0B0yAZq0WfmEDVQ",
...
updateBadges = function() {
    ...
    sendGCMMessageAll({msgId: MSG_IDS.BAGDE_UPDATE, color: color, text: text});   
}
...
sendGCMMessageWithFilter = function(data, filterUserFn) {
     var registrationIds = $.map(Meteor.users.find({}).fetch(), 
                                function(u){
                                    if (filterUserFn(u)) {
                                        return u.profile.registrationId                                        
                                    }
                                });

    sendGCMMessage(registrationIds, data);
}

sendGCMMessageAll = function(data) {
    //var registrationIds = $.map(Meteor.users.find({}).fetch(), function(u){return u.profile.registrationId});
    sendGCMMessageWithFilter(data, function(u) {
            return true;
        });    
}

sendGCMMessage = function(registrationIds, data) {
